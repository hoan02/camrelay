...
function Dashboard() {
  ...
  const { data: events, isLoading: eventsLoading, isError: eventsError } = useQuery({ queryKey: ["events"], queryFn: api.events, refetchInterval: 15_000 });
  ...
  return <>
    ...
    <section className="panel dashboard-wall-panel"><div className="panel-heading"><div><span className="eyebrow">{t("dashboard.liveWall")}</span><h2>{t("dashboard.liveWallTitle")}</h2><p className="panel-description">{t("dashboard.liveWallDescription")}</p></div><NavLink to="/cameras" className="text-link">{t("dashboard.openCameras")} <Icon name="arrow" size={15} /></NavLink></div>{cameras && cameras.length > 0 ? <div className="camera-grid dashboard-wall-grid">{cameras.slice(0, 4).map(camera => <CameraCard key={camera.id} camera={camera} status={statusFor(camera.id)} busy={lifecycle.isPending} onToggle={() => lifecycle.mutate({ id: camera.id, running: statusFor(camera.id) === "running" })} />)}</div> : <div className="wall-empty">{isLoading ? t("common.loading") : isError ? t("dashboard.connectApi") : t("dashboard.noCameras")}</div>}</section>
    <section className="dashboard-grid">
      ...
      <div className="panel activity-panel"><div className="panel-heading"><div><span className="eyebrow">{t("dashboard.recentActivity")}</span><h2>{t("dashboard.activityTitle")}</h2></div><Icon name="activity" size={17} /></div>{eventsLoading ? <div className="activity-state">{t("dashboard.activityLoading")}</div> : eventsError ? <div className="empty-state compact-empty"><span className="empty-icon"><Icon name="activity" /></span><p>{t("dashboard.activityUnavailable")}</p><small>{t("dashboard.activityUnavailableDescription")}</small></div> : events && events.length > 0 ? <><div className="activity-list">{events.slice(0, 5).map(event => <div className={`activity-row ${event.severity}`} key={event.id}><span className="activity-icon"><Icon name={event.kind.startsWith("recording") ? "archive" : "activity"} size={15} /></span><div className="activity-copy"><strong>{event.camera_name}</strong><p>{event.message}</p><small>{event.source} / {formatDate(event.occurred_at)} {formatTime(event.occurred_at)}</small></div><span className="activity-severity">{event.severity}</span></div>)}</div><NavLink to="/recordings" className="activity-link">{t("dashboard.openRecordings")} <Icon name="arrow" size={14} /></NavLink></> : <div className="empty-state compact-empty"><span className="empty-icon"><Icon name="archive" /></span><p>{t("dashboard.noEvents")}</p><small>{t("dashboard.eventsAppear")}</small></div>}</div>
    </section>
  </>;
}
...
function Recordings() {
  ...
  const { data: recordings, isLoading, isError } = useQuery({ queryKey: ["recordings", { cameraId, status }], queryFn: () => api.recordings({ camera_id: cameraId || undefined, status: status || undefined }) });
  const queryClient = useQueryClient();
  ...
  return <><PageHeading eyebrow="MEDIA LIBRARY" title="Recordings" description="Closed segments, local retention, and the path to your cloud archive." /><section className="panel recording-toolbar"><div><span className="eyebrow">{t("recordings.filters")}</span><p>{t("recordings.filterDescription")}</p></div><div className="recording-filters"><label>{t("recordings.camera")}<select value={cameraId} onChange={event => setCameraId(event.target.value)}><option value="">{t("recordings.allCameras")}</option>{cameras?.map(camera => <option value={camera.id} key={camera.id}>{camera.name}</option>)}</select></label><label>{t("recordings.status")}<select value={status} onChange={event => setStatus(event.target.value)}><option value="">{t("recordings.allStatuses")}</option><option value="completed">{t("recordings.completed")}</option><option value="archived">{t("recordings.archived")}</option><option value="uploading">{t("recordings.uploading")}</option><option value="failed">{t("recordings.failed")}</option></select></label></div></section>{isLoading && <div className="panel loading-panel">{t("recordings.loading")}</div>}{isError && <div className="panel empty-panel"><span className="empty-icon"><Icon name="archive" /></span><h2>{t("recordings.apiUnavailable")}</h2><p>{t("recordings.apiUnavailableDescription")}</p></div>}{playing && <section className="panel playback-panel"><div className="panel-heading"><div><span className="eyebrow">{t("recordings.playbackTicket")}</span><h2>{t("recordings.privatePreview")}</h2></div><button className="icon-button" onClick={() => setPlaying(null)} aria-label={t("common.close")}>×</button></div><video className="recording-player" controls autoPlay src={playing.url} /></section>}{recordings && recordings.length === 0 && <div className="panel empty-panel"><span className="empty-icon"><Icon name="archive" /></span><h2>{t("recordings.noSegments")}</h2><p>{t("recordings.noSegmentsDescription")}</p></div>}{recordings && recordings.length > 0 && <div className="panel recordings-list"><div className="recordings-header"><span>{t("recordings.segment")}</span><span>{t("recordings.camera")}</span><span>{t("recordings.status")}</span><span>{t("recordings.size")}</span><span>{t("recordings.actions")}</span></div>{recordings.map(recording => <div className="recording-row" key={recording.id}><div><b>{formatDate(recording.started_at)}</b><small>{recording.kind}{recording.ended_at ? ` - ${formatTime(recording.ended_at)}` : ` - ${t("recordings.inProgress")}`}</small></div><span>{recording.camera_name}</span><span className={`recording-status ${recording.status}`}>{recording.archive_available ? t("recordings.archived") : recording.status}{recording.archive_verified && <small>{t("recordings.verified")}</small>}</span><span title={recording.checksum_sha256 ?? undefined}>{formatBytes(recording.bytes)}<small>{recording.checksum_sha256 ? `${t("recordings.integrity")}: ${recording.checksum_sha256.slice(0, 12)}…` : t("recordings.integrityPending")}</small></span><div className="recording-actions"><button className="text-button" onClick={() => playback.mutate(recording.id)} disabled={playback.isPending}>{t("recordings.play")}</button>{!recording.archive_available && <button className="text-button" onClick={() => archive.mutate(recording.id)} disabled={archive.isPending}>{t("recordings.archive")}</button>}</div></div>)}</div>}{(playback.error || archive.error) && <p className="form-error">{(playback.error instanceof ApiError ? playback.error.message : archive.error instanceof ApiError ? archive.error.message : t("recordings.actionFailed"))}</p>}</>;
}
...
function Settings() {
  ...
  return (
    <>
      <PageHeading eyebrow={t("page.settings.eyebrow")} title={t("page.settings.title")} description={t("page.settings.description")} />
      <div className="settings-grid">
        <section className="panel setting-card">
          <div className="setting-heading"><span className="setting-icon"><Icon name="info" /></span><div><h2>{t("settings.language.title")}</h2><p>{t("settings.language.description")}</p></div></div>
          <div className="choice-group"><button className={locale === "en" ? "choice active" : "choice"} onClick={() => setLocale("en")}>{t("settings.language.english")}</button><button className={locale === "vi" ? "choice active" : "choice"} onClick={() => setLocale("vi")}>{t("settings.language.vietnamese")}</button></div>
        </section>
        <section className="panel setting-card">
          <div className="setting-heading"><span className="setting-icon"><Icon name={theme === "dark" ? "moon" : "sun"} /></span><div><h2>{t("settings.theme.title")}</h2><p>{t("settings.theme.description")}</p></div></div>
          <div className="choice-group"><button className={theme === "dark" ? "choice active" : "choice"} onClick={() => setTheme("dark")}>{t("settings.theme.dark")}</button><button className={theme === "light" ? "choice active" : "choice"} onClick={() => setTheme("light")}>{t("settings.theme.light")}</button></div>
        </section>
        <section className="panel setting-card media-setting">
          <div className="setting-heading"><span className="setting-icon"><Icon name="archive" /></span><div><h2>{t("settings.media.title")}</h2><p>{t("settings.media.description")}</p></div></div>
          {mediaLoading && <p className="setting-status">{t("settings.media.loading")}</p>}
